...
public class LocationFilterTest {
    ...
    public void setUp() throws Exception {
        ...
        Mockito.when(mockLocation.getAccuracy()).thenReturn(TEST_ACCURACY_MARGIN);
        Mockito.when(mockLocation.getTime()).thenReturn(IMMEDIATELY);
        Mockito.when(newLocation.getTime()).thenReturn(SECOND);
        Mockito.when(mockLocation.distanceTo(ArgumentMatchers.any(Location.class))).thenReturn(TEST_DISTANCE_MARGIN);
    ...
    public void isRelevant_shouldReturnFalse_ifLocAccuracyIsLowerThenRequired() throws Exception {
        Mockito.when(newLocation.getAccuracy()).thenReturn(TEST_ACCURACY_MARGIN + 1);
    ...
    public void calcDistance_shouldReturnCorrectResult() throws Exception {
        ...
        Mockito.when(newLocation.getTime()).thenReturn(SECOND);
        Mockito.when(mockLocation.distanceTo(ArgumentMatchers.any(Location.class))).thenReturn(distanceTnMeters);
    ...
    public void isRelevant_shouldReturnFalse_ifVelocityIsHigherThanLimit() throws Exception {
        Mockito.when(mockLocation.distanceTo(ArgumentMatchers.any(Location.class))).thenReturn(TEST_DISTANCE_MARGIN); // ~36 km/h
        long timeHalfSecond = 550L;
        Mockito.when(newLocation.getTime()).thenReturn(timeHalfSecond);
        assertFalse(testSubject.isRelevant(newLocation));
    }
    ...
    public void isRelevant_shouldReturnFalse_ifNewLocationUpdateIsOlderThanThePrev() throws Exception {
        ...
        Mockito.when(mockLocation.getTime()).thenReturn(1L);
        Mockito.when(newLocation.getTime()).thenReturn(IMMEDIATELY);
        assertFalse(testSubject.isRelevant(newLocation));
    }

    @Test
    public void isRelevant_shouldReturnFalse_ifNewLocationsDistanceIsSmallerThanDistanceMargin() {
        Mockito.when(mockLocation.distanceTo(ArgumentMatchers.any(Location.class))).thenReturn(TEST_DISTANCE_MARGIN - 1);
        Mockito.when(newLocation.getTime()).thenReturn(300L);
        assertFalse(testSubject.isRelevant(newLocation));
    }

    @Test
    public void isRelevant_shouldReturnTrue_ifRelevant() {
        Mockito.when(newLocation.getTime()).thenReturn(10000L);
